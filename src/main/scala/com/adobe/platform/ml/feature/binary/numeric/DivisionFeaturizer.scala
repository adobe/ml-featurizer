...
private[feature] trait DivisionFeaturizerParams extends Params with HasInputCols with HasOutputCol {
...
class DivisionFeaturizer(override val uid: String)
  extends Transformer with DivisionFeaturizerParams with DefaultParamsWritable {

  def this() = this(Identifiable.randomUID("divisionFeaturizer"))
  ...
  override def copy(extra: ParamMap): DivisionFeaturizer = defaultCopy(extra)
}

object DivisionFeaturizer extends DefaultParamsReadable[DivisionFeaturizer] {
  override def load(path: String): DivisionFeaturizer = super.load(path)
}