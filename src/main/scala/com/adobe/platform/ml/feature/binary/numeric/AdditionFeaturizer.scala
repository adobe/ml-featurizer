...
private[feature] trait AdditionFeaturizerParams extends Params with HasInputCols with HasOutputCol {
...
class AdditionFeaturizer(override val uid: String)
  extends Transformer with AdditionFeaturizerParams with DefaultParamsWritable {

  def this() = this(Identifiable.randomUID("additionFeaturizer"))
  ...
  override def copy(extra: ParamMap): AdditionFeaturizer = defaultCopy(extra)
}

object AdditionFeaturizer extends DefaultParamsReadable[AdditionFeaturizer] {
  override def load(path: String): AdditionFeaturizer = super.load(path)
}