...
class HourOfDayFeaturizer(override val uid: String)
  extends Transformer with HourOfDayFeaturizerParams with DefaultParamsWritable {

  def this() = this(Identifiable.randomUID("hourOfDayFeaturizer"))
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    inputType match {
      case StringType => {
        dataset.select(col("*"), toHourOfDayString(col($(inputCol))).as($(outputCol), metadata))
      }
      case TimestampType => {
        dataset.select(col("*"), toHourOfDayTimestamp(col($(inputCol))).as($(outputCol), metadata))
      }
    }
  }
  ...
  override def copy(extra: ParamMap): HourOfDayFeaturizer = defaultCopy(extra)
}

object HourOfDayFeaturizer extends DefaultParamsReadable[HourOfDayFeaturizer] {
  override def load(path: String): HourOfDayFeaturizer = super.load(path)
}